...
class MainWindow(QMainWindow):
    ...
    def _on_generate_bin(self) -> None:
        ...
        for filename, offset, size in report.rom_offsets:
            self._log_line(f"  - {filename}: offset 0x{offset:06X}, {size} bytes")
        self._log_line(f"  - Padding (0xFF): {report.padding_size} bytes")
